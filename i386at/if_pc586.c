 ...
 * HISTORY
 * $Log:	if_pc586.c,v $
 * Revision 1.8.1.16  91/07/15  00:17:10  rvb
 * 	Kill nuisance print out.
 * 	[91/07/09            rvb]
 * 
 * Revision 1.8.1.15  91/03/28  08:46:12  rvb
 * 	The change below should also be applied to the requeue loop termination
 * 	detection logic.
 * 	[91/03/12            rvb]
 * 
 * Revision 1.8.1.14  91/03/01  22:43:46  rvb
 * 	Well, we really need to check both RBD_SW_EOF AND 0xffff and
 * 	other fields can be 0xffff besides status.
 * 	[91/03/01            rvb]
 * 
 * Revision 1.8.1.13  91/01/30  08:47:27  rvb
...
/*
	if ((scb_p->scb_status & SCB_SW_INT) != SCB_SW_CNA) {
		printf("pc%d config(): unexpected final state %x\n",
			unit, scb_p->scb_status);
	}
*/
	pc586ack(unit);
