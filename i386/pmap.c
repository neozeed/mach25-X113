 ...
 * HISTORY
 * $Log:	pmap.c,v $
 * Revision 1.8.1.10  91/05/03  10:57:45  rvb
 ...
 * 	[91/05/03            rvb]
 * 
 * 	Need more MOREVM for X134.
 * 	[91/03/25            rvb]
 * 
 * Revision 1.8.1.9  90/05/14  13:20:07  rvb
 * 	Include kupfer's code sans pmap_phystokv
 * 	[90/05/04            rvb]
 * 
 * 	pmap_collect had to major errors.  Picked up fixes from dbg
 * 	 1. it only freed wired pages
 * 	 2. it kmemfree'd phystokv stuff -- a no no.
 * 	[90/05/01            rvb]
 * 
 * Revision 1.16  90/05/01  21:28:54  kupfer
 * Implement copy-on-reference.
 *
 * Revision 1.8.1.8  90/03/12  11:17:23  rvb
 * 	Corrected userwrite and moved it to pmap.c
 * 	[90/03/12            rvb]
 * 
 * Revision 1.8.1.7  90/03/10  00:27:03  rvb
 * 	Fix pmap_pte loops to check pde first, before checking entries on
 * 	that page.
 * 	[90/03/07            rvb]
 * 
 * 		I386e- merge. [kupfer]
 * 
 * Revision 1.8.1.5  90/02/28  15:48:26  rvb
 * 	ORC=>M380 [kupfer]
 * 
 * Revision 1.8.1.4  90/02/27  08:47:16  rvb
 * 	Define pmap_valid_page() so that the it gives the correct answer
 * 	for any page from 0 to "memsize".  This means you account for
 * 	the hole and kernel text.
 * 	[90/02/19            rvb]
 * 
...
#ifndef	THREAD_EXIT
	extern boolean_t	stack_swapout_allowed;
	stack_swapout_allowed = FALSE;
#endif	THREAD_EXIT
 ...
 */
#ifdef	THREAD_EXIT
#define MOREVM	30			/* megabytes */
#else	THREAD_EXIT
#define MOREVM	40			/* megabytes */
#endif	THREAD_EXIT
	...
	register pmap_t	p;
{
	...
	if (c == 0) {
		 ...
		 *	pmap structure.
		 */
		for (pdep = p->cr3; pdep < &p->cr3[pdenum(VM_MIN_KERNEL_ADDRESS)]; pdep++) {
			if (pdep->valid == 0)
				continue;
...
Retry:
	...
	if (old_pa == pa) {
		 ...
		 */
		pmap_remove_range(pmap, v, v + i386_ptob(ptes_per_vm_page));
	    }
	...
	pmap_t 		p;
{
	...
	maxidx = pdenum(VM_MIN_KERNEL_ADDRESS);
	for (pdp = p->cr3; pdp < &p->cr3[maxidx]; pdp++) {
		if (pdp->valid == 0)
			continue;
		{
		    register vm_offset_t pa;
...
#define pte_to_pa(x) ((x) & (~PTEMASK))
		    pa = pte_to_pa(*(int *)pdp);
		    pv_h = pai_to_pvh(pa_index(pa));
		    while (pv_h != PV_ENTRY_NULL && pv_h->pmap != kernel_pmap)
			pv_h = pv_h->next;
		    if (pv_h == PV_ENTRY_NULL)
		...
		if (!wired) {
			...
			pmap_remove_range(p, va, va + pdetova(1));
			*(int *)pdp = 0;
